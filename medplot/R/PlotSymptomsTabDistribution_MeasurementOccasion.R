...
plotDistribution <- function (data, selectedSymptoms, selectedProportion, measurements ) {
  ...
  prop.with.symptoms=apply(data[measurements==selectedProportion, ,drop=FALSE],
...
plotDistributionBoxplot <- function (data,
  ...
  if(!posOnly) {
    boxplot(t(apply(data[measurements==selectedProportion, selectedSymptoms[my.order.symptoms]],
                    1, function(x) x)), 
    ...
    tmp=(apply(data[measurements==selectedProportion, selectedSymptoms[my.order.symptoms]],
               1,  function(x) x))
...
#' 
#' @param data.yn Data with threshold applied.
#' @param selectedSymptoms Column names of symptoms in data frame.
#' @param selectedProportion User selected measurement time.
#' @param measurements Vector of measurement values.
#' 
...
plotCI <- function (data.yn,
  ...
  for.CI=t(apply(data.yn[measurements==selectedProportion,],
                 2, function(x) {a=prop.test(sum(x==1, na.rm=T), length(x))
  ...
  for(i in 1:num.symptoms){
    ...
    segments( for.CI[,4], 1:num.symptoms, for.CI[,5], 1:num.symptoms)
...
#' @title Table with descritpive statistics for both groups 
#' 
#' @description Thresholds taken into account for proportions, not for medians.
#' 
#' @param data The data
#' @param measurementVar The mesurement variable.
#' @param forMeasurement For which measurement occasion.
#' @param symptomsNames For which outcomes.
#' @param thresholdValue Threshold for dichotomizing.

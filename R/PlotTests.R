...
# import test data set; do not check for valid variable names! (to allow for dates)
#myData <- read.delim("./data/data2.txt", header=TRUE, check.names=FALSE, na.strings=c("NA",""))
# head(myData)

# import parameters for function

# myParameters <- data.frame(cbind(Result=c("CoV", "neg","INF A","Other","RSV A"),
#                                 Color=c("blue", "green", "red", "purple", "black"),
#                                 Size=c(2, 1, 2, 2, 2) ) )
#myParameters <- read.delim("./data/parameters.txt", header=TRUE)


### TEST FUNCTION ####

#data <- myData
#figureParameters <- myParameters

# drawFigures(myData,myParameters,graphsDir,generateTooltips)

### Functions to benchmark speed of execution
# run function to test it
# library(compiler)
# library(rbenchmark)
# library(profr)
# addToolTipscompiled <- cmpfun(addToolTips)
# drawFigurescompiled <- cmpfun(drawFigures)
# xmlParsecompiled <- cmpfun(xmlParse)
# benchmarkResults <- benchmark(drawFigures(myData,myParameters,graphsDir,generateTooltips),replications=1)
# benchmarkResults <- profr(drawFigures(myData,myParameters,graphsDir,generateTooltips),0.1)
# Cairo(paste(graphsDir, "/", "benchmark.svg", sep=""), type="svg",width=300,height=50,units="cm",dpi=300)
# plot(benchmarkResults)
# dev.off()

# func1 <- function (x) {1/(1+x)}
# func1compiled <- cmpfun(func1)
# benchmark(func1(2), replications=1000000)
# benchmark(func1compiled(2), replications=1000000)
...
# 
# library(seriation)
# myTestResults <- array(myData[,6:21], dim=c(97,16))
# myTestResults <- as.array(myTestResults)
# xtabs(myData[,6:21])
# 
# seriate(myTestResults)
# 
# tableofGroups <- table(data$Type, data$Diagnosis)[1,1]

# testDataFrame <- data.frame(x=c(1,2,3,4,5),y=c(1,2,"3,4",4,5))
# dev.off()
# par("usr")
# plot(testDataFrame, col=c("blue","blue","blue","blue","blue"))
# str(testDataFrame)

# pointSvg <- getPlotPoints(doc, addTypes=TRUE)
# dput(pointSvg, file="pointSVG")
# dump(pointSvg, file="pointSVG")
# dev(pointSVG.txt)
# sink("pointSVG")
# dev.off()     
